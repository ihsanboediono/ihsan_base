							...
							<div class="card">
                                ...
                                <form action="{{ route('admin.news.store') }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="title_id" name="title_id" placeholder="tuliskan judul berita" value="{{ old('title_id') }}">
                                        @error('title_id') <span class="text-danger">{{ $message }}</span> @enderror
                                    </div>
                                    {{-- <div class="form-group">
                                        <label for="title_en">Judul berita (English)</label>
                                        <input type="text" class="form-control" id="title_en" name="title_en" placeholder="Judul berita bahasa inggris" value="{{ old('title_en') }}">
                                        ...
                                        <div class="form-group col-md">
                                            ...
                                            <select class="form-control" name="category" id="category" style="width: 100%" value="{{ old('category') }}">
                                                ...
                                                @foreach ($categories as $category)
                                                    <option value="{{ $category->id }}" {{ old('category') == $category->id ? "selected" : "" }}>{{ $category->name_id }} | {{ $category->name_en }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" id="editor1" rows="10" name="description_id" placeholder="">{{ old('description_id') }}</textarea>
                                        @error('description_id') <span class="text-danger">{{ $message }}</span> @enderror
                                    </div>
                                    {{-- <div class="form-group">
                                        <label for="description_en">Deskripsi Berita (English)</label>
                                        <textarea class="form-control" id="editor2" rows="10" name="description_en" placeholder="Write News description">{{ old('description_en') }}</textarea>
                                    ...
                                    <a href="{{ route('admin.news.index') }}" type="button" class="btn btn-warning btn-rounded ml-2">Kembali</a>
