...
@section('title',  'Edit susunan pengurus')
						...
						<div class="col-md-8">
							<div class="card">
                                <div class="card-header card-info">
                                    <div class="card-title">Form Edit Manajemen</div>
                                </div>
								<div class="card-body">
                                    <form action="{{ route('admin.management.update', ['management'=>$management->id]) }}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')
                                        <div class="form-group">
                                            <label for="name">Nama</label>
                                            <input type="text" class="form-control" id="name" name="name" placeholder="Nama" value="{{ old('name', $management->name) }}">
                                            @error('name') <span class="text-danger">{{ $message }}</span> @enderror
                                        </div>
                                        <div class="form-group">
                                            <label for="position_id">Jabatan</label>
                                            <input type="text" class="form-control" id="position_id" name="position_id" placeholder="Jabatan" value="{{ old('position_id', $management->position_id) }}">
                                            @error('position_id') <span class="text-danger">{{ $message }}</span> @enderror
                                        </div>
                                        {{-- <div class="form-group">
                                            <label for="position_en">Jabatan (English)</label>
                                            <input type="text" class="form-control" id="position_en" name="position_en" placeholder="Jabatan bahasa inggris" value="{{ old('position_en', $management->position_en) }}">
                                            @error('position_en') <span class="text-danger">{{ $message }}</span> @enderror
                                        </div> --}}
                                        <div class="form-group col-md-4">
                                            <p style="font-weight: 600">Gambar</p>
                                            <img class="img-preview img-fluid mb-3" src="{{ $management->image_url }}">
                                            <p id="file-name"></p>
                                        ...
                                        <button class="btn btn-primary btn-rounded" type="submit">Edit manajemen</button>
