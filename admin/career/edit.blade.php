						...
						<div class="col-md-8">
							<div class="card">
                                <div class="card-header card-info">
                                    <div class="card-title">Form edit lowongan pekerjaan</div>
                                </div>
								<div class="card-body">
                                <form action="{{ route('admin.career.update', ['career' => $career->id]) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    @method('put')
                                    <div class="form-group">
                                        <label for="title_id">Nama lowongan pekerjaan</label>
                                        <input type="text" class="form-control" id="title_id" name="title_id" placeholder="Nama lowongan pekerjaan" value="{{ old('title_id', $career->title_id) }}">
                                        @error('title_id') <span class="text-danger">{{ $message }}</span> @enderror
                                    </div>
                                    {{-- <div class="form-group">
                                        <label for="title_en">Nama lowongan pekerjaan (English)</label>
                                        <input type="text" class="form-control" id="title_en" name="title_en" placeholder="Nama lowongan pekerjaan bahasa inggris" value="{{ old('title_en', $career->title_en) }}">
                                        @error('title_en') <span class="text-danger">{{ $message }}</span> @enderror
                                    </div> --}}
                                    <div class="form-group row">
                                        <div class="form-group col-md">
                                            <label for="start_date">Mulai</label>
                                            <input type="date" class="form-control" id="start_date" name="start_date" value="{{ old('start_date', $career->start_date) }}">
                                            @error('start_date') <span class="text-danger">{{ $message }}</span> @enderror
                                        </div>
                                        <div class="form-group col-md">
                                            <label for="end_date">Selesai</label>
                                            <input type="date" class="form-control" id="end_date" name="end_date" value="{{ old('end_date', $career->end_date) }}">
                                            @error('end_date') <span class="text-danger">{{ $message }}</span> @enderror
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label for="description_id">Deskripsi lowongan pekerjaan</label>
                                        <textarea class="form-control" id="editor1" rows="5" name="description_id" placeholder="">{{ old('description_id', $career->description_id) }}</textarea>
                                    ...
                                    {{-- <div class="form-group">
                                        ...
                                        <textarea class="form-control" id="editor2" rows="5" name="description_en" placeholder="">{{ old('description_en', $career->description_en) }}</textarea>
                                        @error('description_en') <span class="text-danger">{{ $message }}</span> @enderror
                                    </div> --}}
                                    {{-- <div class="form-group col-md-4">
                                        <p style="font-weight: 600">Gambar</p>
                                        <img src="{{ $career->image_url }}" class="img-preview img-fluid mb-3">
                                        <p id="file-name"></p>
                                    ...
                                    <button class="btn btn-primary btn-rounded">Edit lowongan pekerjaan </button>
